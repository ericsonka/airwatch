<script>
    ...
    async function add_device(e){
        ...
        // Validate location_coordinates
        if (!new_device.location_coordinates) {
  ...
  async function initAutocomplete() {
    ...
    //   website_data.function_data[index].function_google_map_location_link = googleMapsLink[index]
    //   location[index] = place.formatted_address;
    ...
    <div class="main_section">
        ...
        {#each devices_data_list as device}
            <div class="added_device_list">
                ...
                <h4>Device Status : <span class:is_online={device.device_status != 'offline'}>{device.device_status}</span></h4>
                    <i on:click={() => handle_delete_bus_profile(device.device_profile_id)} class="fa-solid fa-trash delete"></i> 
                    <!-- <pre>{device._id}</pre> -->
        ...
        {#if show_hide}  
            <div class="adding_page_container">  
                <i class="fa-solid fa-xmark" on:click={show_adding_device_popup}></i>

                <div class="ading_page_details">
                    <h1>Add Device</h1>
                    <div class="input-container">
                        <form action="">
                            <input placeholder="Enter Device Id" bind:value={new_device.device_id} class="input-field" type="number">
                            <label for="input-field" class="input-label">Enter Device Id</label>
                            <input placeholder="Device Location coordinates" bind:value={new_device.location_coordinates} class="input-field" type="text">
                            <label for="input-field" class="input-label">Device Location</label>
                            <input placeholder="Device Name" bind:value={new_device.device_name} class="input-field" type="text">
                            <!-- <label for="input-field" class="input-label"></label> -->
                            <input placeholder="Location Name" bind:value={new_device.location_name} class="input-field" type="text">
                            
                                <label for="function_google_map_location_link">Location Name </label>
                                <input type="text"  id="location-input{index}"
                                bind:this={current_input_element[0]}
                                bind:value={location[index]}
                                on:input={initAutocomplete}
                                placeholder="Search Your Place">
                           
                            <p>Device Status</p>
                            <select name="status" id="status" bind:value={new_device.device_status} class="status-select">
                            ...
                            <button class="cta" on:click={add_device} >
                                <span class="hover-underline-animation" > Add device </span>
                                <svg
                                  id="arrow-horizontal"
